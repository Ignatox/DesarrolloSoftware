...
//Asi filtramos por producto
const input: string ='bag'; //este en lower case igual que el de abajo
const filteredProducts = Productos.filter((item) => {
    return item.description.toLowerCase().includes(input) || item.title.toLowerCase().includes(input) //Siempre que compara todo a minuscula o todo mayuscula
}); //Lo que yo escriba lo incluya o en el titulo o en la descripción 

console.log('filteredProducts', filteredProducts);

const productosToSorted = Productos.filter((item) => {
    return Productos.t
})